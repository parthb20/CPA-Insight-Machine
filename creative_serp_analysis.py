...
COL_MAP = {
    ...
    'Actual Advertiser Payout': 'actual_adv_payout',  # ADD
    'Mnet ROAS': 'mnet_roas',  # ADD
    'Advertiser ROAS': 'adv_roas'  # ADD
}
...
def update_creative_serp(advs, camp_types, camps):
    ...
    return (stats_display, best_creatives, worst_creatives, 
            best_serps, worst_serps, best_pairs, worst_pairs)
    ...
    for attr in ATTRIBUTE_COLUMNS:
        ...
        attribute_sections.append(
            html.Div([
                ...
                html.Div(id=drill_down_id, style={'marginTop': '20px'})
            ])
        )
...
for attr in ATTRIBUTE_COLUMNS:
    @callback(
        Output(f'drilldown_{attr}', 'children'),
        [Input(f'table_{attr}', 'selected_rows'),
         Input('cs_adv_dd', 'value'),
         Input('cs_camp_type_dd', 'value'),
         Input('cs_camp_dd', 'value')],
        prevent_initial_call=True
    )
    def update_drilldown(selected_rows, advs, camp_types, camps, attr=attr):
        ...
        selected_value = agg_data.iloc[selected_rows[0]][attr]
        avg_cvr_for_value = agg_data.iloc[selected_rows[0]]['cvr']
        
        # Filter ad titles for this attribute value
        ...
        # Create style_data_conditional for highlighting
        style_conditional = [
            {
                'if': {
                    'filter_query': f'{{cvr}} > {avg_cvr_for_value}',
                    'column_id': 'cvr'
                    },
                'backgroundColor': '#1a4d2e',
                'color': '#00ff00',
        ...
        return html.Div([
            ...
            html.P(f"Average CVR for this category: {avg_cvr_for_value:.2f}% (Rows in green have CVR > this average)", 
