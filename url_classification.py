...
# CONFIG
# =========================================================
DATA_FILE = "URL_Full File_02Dec2025_16Dec2025.csv"
...
df['roas'] = np.where(df['max_cost']>0, df['adv_value']/df['max_cost'], np.nan)
...
# =========================================================
STOP_WORDS = {
    'com','www','http','https','html','php','aspx','jsp',
...
def weighted_aggregate(df, col):
    ...
    g['roas'] = np.where(g['max_cost']>0, g['adv_value']/g['max_cost'], np.nan)
...
def create_treemap(g, metric_color, metric_sort, title, show_cvr_ctr=True, top_n=10, col_name='concepts', avg_metrics=None):
    ...
    if show_cvr_ctr:
        ...
        g['hover_text'] = (
            ...
            'ROAS: ' + g['roas'].round(2).astype(str)
        )
...
def bubble_chart(g, x_col, metric, hover_map, title, top_n=5):
    ...
    g['hover_text'] = (
        ...
        'CPA: ' + g['cpa'].round(2).astype(str) + '<br>' +
        'ROAS: ' + g['roas'].round(2).astype(str) + '<br><br>' +
...
layout = dbc.Container(fluid=True, style={'backgroundColor': '#111'}, children=[
        ...
        dbc.Col([
            ...
            dash_table.DataTable(
                ...
                style_data_conditional=[
                    {
                        'if': {
                            'filter_query': '{cvr_vs_avg} > 0',
                            'column_id': 'cvr_vs_avg'
                        },
                        'color': '#00ff00',
                        'fontWeight': 'bold'
                    },
    ...
    html.P("Shows performance by contextuality level (e.g., Highly Contextual, Contextual, etc.)", 
           style={'color': '#aaa', 'fontSize': '12px', 'marginBottom': '10px'}),
    dbc.Row([
        dbc.Col(dcc.Loading(dcc.Graph(id='ctx_cvr'), type="circle", color="#5dade2"), width=6),
        dbc.Col(dcc.Loading(dcc.Graph(id='ctx_ctr'), type="circle", color="#5dade2"), width=6)
    ]),
    dbc.Row([
        dbc.Col(dcc.Loading(dcc.Graph(id='ctx_cpa'), type="circle", color="#5dade2"), width=6),
        dbc.Col(dcc.Loading(dcc.Graph(id='ctx_roas'), type="circle", color="#5dade2"), width=6)
    ]),
...
@callback(
    [Output('agg_stats', 'children'),
     ...
     Output('ctx_cvr', 'figure'),
     Output('ctx_ctr', 'figure'),
     Output('ctx_cpa', 'figure'),
     Output('ctx_roas', 'figure'),
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        html.Div([
                            ...
                            html.Span(f"{agg_roas:.2f}", style={'color': '#00ff00', 'fontSize': '18px'})
                        ])
                    ], width=2)
                ])
            ])
...
# Treemaps: CVR-CTR (sorted by CTR desc, colored by CVR) - TOP 10 DEFAULT
        avg_metrics = {'cvr': agg_cvr, 'ctr': agg_ctr, 'cpa': agg_cpa, 'roas': agg_roas}
...
# Treemaps: ROAS-CPA (sorted by CPA desc, colored by ROAS) - TOP 10 DEFAULT
        tree_roas_cpa = create_treemap(g_concept_top, 'roas', 'cpa', 
        ...
        if len(best_concepts_display) > 0:
            ...
            best_concepts = best_concepts_display[['concepts', 'clicks', 'conversions', 'cvr', 'avg_cvr', 'cvr_vs_avg', 'ctr', 'cpa', 'roas']].round(2).to_dict('records')
        else:
            best_concepts = []
            
        if len(worst_concepts_display) > 0:
            worst_concepts_display['avg_cvr'] = agg_cvr
            worst_concepts = worst_concepts_display[['concepts', 'clicks', 'conversions', 'cvr', 'avg_cvr', 'cvr_vs_avg', 'ctr', 'cpa', 'roas']].round(2).to_dict('records')
        else:
            worst_concepts = []
        ...
        if len(best_urls_display) > 0:
            ...
            best_urls = best_urls_display[['url', 'clicks', 'conversions', 'cvr', 'avg_cvr', 'cvr_vs_avg', 'ctr', 'cpa', 'roas']].round(2).to_dict('records')
        else:
            best_urls = []
            
        if len(worst_urls_display) > 0:
            worst_urls_display['avg_cvr'] = agg_cvr
            worst_urls = worst_urls_display[['url', 'clicks', 'conversions', 'cvr', 'avg_cvr', 'cvr_vs_avg', 'ctr', 'cpa', 'roas']].round(2).to_dict('records')
        else:
            worst_urls = []
        ...
        metrics = ['cvr', 'ctr', 'cpa', 'roas']
        
        # Show all contextuality items (no top_n limit)
        ctx_figs = [bubble_chart(g_contextuality, 'contextuality', m, h_contextuality, f'Contextuality - {m.upper()}', len(g_contextuality)) for m in metrics]
        ...
        tree_url_top_roas_cpa = create_treemap(g_url_top, 'roas', 'cpa', 
        ...
        tree_dom_top_roas_cpa = create_treemap(g_dom_top, 'roas', 'cpa', 
        ...
        tree_url_final_roas_cpa = create_treemap(g_url_final, 'roas', 'cpa', 
        ...
        tree_dom_final_roas_cpa = create_treemap(g_dom_final, 'roas', 'cpa', 
        ...
        return (
            ...
            *ctx_figs,  # NEW - 4 contextuality figures
        ...
        return tuple([empty_stats] + [empty]*2 + [[]]*4 + [empty]*28)
@callback(
